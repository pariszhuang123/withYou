...
| [Repository Contract](docs/contracts/call-session-repository.md) | Session + event persistence | Touching database |
| [Content Resolver Contract](docs/contracts/content-resolver.md) | Asset resolution logic | Touching content/assets |
...
| [Content Model](docs/content-model.md) | Scenario × caller mapping (Chinese only, no personas) | Touching assets/content |
...
2. **Write tests before or alongside code.** Every new file must have a corresponding test file. No exceptions.
3. **Run `flutter analyze` and `flutter test` before declaring done.** Zero warnings. Zero test failures.
4. **Use dependency injection.** All contract implementations are registered in `lib/di/service_locator.dart`. Screens and blocs receive contracts via constructor injection.
...
7. **Handle all call flow states explicitly.** Every combination of accept/decline/miss for every stage must be handled. No implicit fallthrough.
...
4. **Do not store any user-identifiable information.** No analytics IDs, device IDs, or PII.
